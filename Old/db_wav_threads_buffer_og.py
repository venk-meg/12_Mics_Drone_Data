...
class AudioPublisher(Node):
    def __init__(self, device_ports,numberofmics, sample_rate = 44100, duration = .1):
        ...
        self.output_filename = "audiocaptures/audiocaptures_12/withthreads/output_audio"
        self.reference_amplitude = 1 #change as per requirements
        self.buffertime = 10 #10 seconds buffer time
    ...
    def startrec(self, deviceport, deviceportindex):
        audio_stream = sd.InputStream(
            ...
            blocksize=420, #factor of 44100, keeping it fixed to not lose any samples
        ...
        while not audio_stream.active:
           self.get_logger().info(f"audio stream for device{deviceportindex} not working")
           pass
    ...
    def audio_callback(self, deviceportindex,indata, frames, time, status):
        ...
        with self.audiodata_lock[deviceportindex]:
            ...
            if self.write_position[deviceportindex] >= self.audiodata[deviceportindex].shape[0]:
                self.save_buffer(deviceportindex)
                self.reset_buffer(deviceportindex)

        # update indata to db data list
        with self.dbdata_lock[deviceportindex]:
            if self.dbdata[deviceportindex] is not None:
                self.dbdata[deviceportindex]= np.vstack((self.dbdata[deviceportindex], indata))
            else:
                self.dbdata[deviceportindex] = indata
    
    def save_buffer(self, deviceportindex):
    ...
    def reset_buffer(self, deviceportindex):
    ...
    def publish_db_data(self):
        ...
        for i in range(len(self.dbdata_lock)):
            with self.dbdata_lock[i]:
                if self.dbdata[i] is not None:
                    rms_amplitude = np.sqrt(np.mean(np.square(self.dbdata[i])))
                    db = 20 * np.log10(rms_amplitude / self.reference_amplitude)
                    msg.data.append(float(db))
                    self.dbdata[i] = None  
                else:
                    self.get_logger().info(f"db data for mic{i+1} not obtained")
        #publish message
        ...
        for i in range(len(self.audiodata_lock)):
            with self.audiodata_lock[i]:
                ...
                try:
                    ...
                    temp_files = [f"{self.output_filename}_{i + 1}_{buffer_idx}_TEMP.wav" for buffer_idx in range(1, self.buffer_num[i])]
...
def main():
    ...
    audio_publisher = AudioPublisher(deviceports,11)
